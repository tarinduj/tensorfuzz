import json
from transformer import Config, Transformer, MAX_LEN
import numpy as np
import jax
...
def make_model(config: Config):
    if config["block_type"] == "MLP":
        return Transformer(
            ...
            d_ff=config['d_ff']  # Add this line
        )
...
def generate(prompt: str, model: Transformer, params: dict, enc: Encoding):
    fast_apply = jax.jit(model.__call__)
...
config = Config(
    ...
    n_heads=8,
    n_layers=1,
    d_ff=512,  # Add this line
    learning_rate=1e-5,
    batch_size=16,
    weight_decay=0.001,
    block_type="MLP", 
    max_steps=10000  # Add this if it's not already in your Config
)
