...
def seed_corpus_from_numpy_arrays(
    ...
    Args:
      ...
      fetch_function: grabs output from JAX runtime.
    Returns:
      List of CorpusElements.
    """
    seed_corpus = []
    _, _, coverage_batches, metadata_batches = fetch_function(state, prompt_tokens_array, mask)
    ...
    new_element = CorpusElement(
        (state, prompt_tokens_array, mask), metadata_list[0], coverage_list[0], None
    )
...
class Updater(object):
    ...
    def build_index_and_flush_buffer(self, corpus_object):
        ...
        if self.lookup_array.dtype == np.float16:
            self.lookup_array = self.lookup_array.astype(np.float32)
        self.flann.build_index(self.lookup_array, algorithm=self.algorithm)
        print("Flushing buffer and building index.")
    ...
    def update_function(self, corpus_object, element):
        ...
        if corpus_object.corpus is None:
            ...
            if nearest_distance > self.threshold:
                print(
                    f"corpus_size {len(corpus_object.corpus)} mutations_processed {corpus_object.mutations_processed}"
                )
                print(
                    f"coverage: {element.coverage}, metadata: {element.metadata}"
                )
